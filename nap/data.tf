...
data "kubernetes_service_v1" "nginx-service" {
  metadata {
    name = try("llm-nap-fcd5-nginx-ingress", format("%s-%s", helm_release.nginx-plus-ingress.0.name, helm_release.nginx-plus-ingress.0.chart), format("%s-%s", helm_release.nginx-plus-ingresslink.0.name, helm_release.nginx-plus-ingresslink.0.chart))
